...
const BAILEYS_BROWSER: [string, string, string] = [
  process.env.BAILEYS_BROWSER_NAME?.trim() || 'Ubuntu',
  'Chrome',
  '120.0.0',
];
  ...
  private async connectInner(): Promise<void> {
    ...
    const sock = b.default({
      ...
      shouldSyncHistoryMessage: () => {
        if (process.env.OUTBOUND_ONLY === 'true') return false;
        return process.env.BAILEYS_SYNC_HISTORY !== 'false';
      },
      syncFullHistory: process.env.BAILEYS_SYNC_FULL_HISTORY === 'true' && process.env.OUTBOUND_ONLY !== 'true',
      ...
      markOnlineOnConnect: process.env.OUTBOUND_ONLY === 'true'
        ? false
        : process.env.BAILEYS_MARK_ONLINE_ON_CONNECT !== 'false',
