...
export const TABLE_IMPORTERS: AnyTableImporter[] = [
  ...
  defineTableImporter({
    ...
    // A session name is still weighed against the on-disk auth directories (the boot migration and
    // the legacy purge on delete), so an unvalidated imported name (this path bypasses
    // CreateSessionDto) could traverse the filesystem. Skip + warn instead of throwing, so one bad
    // row doesn't 500 the whole restore.
