...
> `503` and the request was not carried out, so it is safe to retry. Once the request has
> been sent, a failure answers `504` (no reply within `SESSION_PROXY_TIMEOUT_MS`) or `502`
> (the connection broke mid-request): the owner may already have carried it out, so a
> non-idempotent call such as a message send must not be repeated blindly on either.
>
